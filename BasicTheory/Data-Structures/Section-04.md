...
上一小节我们学习了朴素模式匹配算法。本小节将基于该算法进行优化，得到 **KMP算法**。算法名称虽看似复杂，但实际只是因三位发明者（Knuth、Morris、Pratt）姓氏首字母而得名，不必畏惧。其本质就是对朴素模式匹配算法的改进。

#### 一、回顾朴素模式匹配的缺陷

朴素算法的思路是：将主串中所有与模式串等长的子串，依次与模式串逐字符比较，直到找到第一个完全匹配的子串。若当前子串在某位置失配，则转而比较下一个子串，且从该子串的第一个字符重新开始。

为什么必须从头开始？因为在失配之前，我们并不清楚主串中那些字符的具体内容，只能逐一尝试。

但事实上，**当某个子串匹配到最后一个字符才失配时，主串前面那些字符的内容已经可以确定**——它们必然与模式串的对应位置一致。这些已知信息，朴素算法完全没有利用，造成了大量重复比较。

#### 二、利用已知信息跳过无效子串

我们以一个具体的模式串为例：`"ABCABC"`（实际原语音中模式串为 `"ABABBC"` 之类，但核心逻辑一致，此处按通用情况描述）。假设模式串长度为6，字符依次为 `A B C A B C`。

**情形1**：当第6个字符失配时，前5个字符（`A B C A B`）与主串对应位置完全匹配。此时，以主串第2个字符开头的子串，其第一个字符与模式串的 `A` 能否匹配？我们已知主串第2个字符是 `B`，而模式串第1个字符是 `A`，显然不匹配，所以该子串无需比较。同理，以第3个字符开头的子串，第一个字符是 `C`，也不匹配。以第4个字符开头的子串，第一个字符是 `A`，可以匹配；第二个字符是 `B`，也可以匹配；但第三个字符我们尚不知晓（因为它正是刚才失配的那个位置）。因此，**我们可以直接跳过一个、二个字符开头的子串，并从第4个字符开始，直接比较模式串的第3个字符与失配位置**。

结论：当第6个字符失配时，主串指针 `i` 不变，模式串指针 `j` 应回到 `j = 3`（即从模式串第3个字符继续比较）。

**情形2**：若第5个字符失配，则前4个字符已知为 `A B C A`。检查以第2个字符开头的子串，第一个字符是 `B`，不匹配；以第3个字符开头的子串，第一个字符是 `C`，不匹配；以第4个字符开头的子串，第一个字符是 `A`，可以匹配，但第二个字符是 `B`（已知），与模式串第2个字符 `B` 也匹配，而失配位置（主串第5个字符）的内容未知。因此，应保持 `i` 不变，令 `j = 2`（从模式串第2个字符开始比较）。

**情形3**：第4个字符失配，前3个字符为 `A B C`。以第2个字符开头的子串，首字符 `B` 不匹配；以第3个字符开头的子串，首字符 `C` 不匹配；以第4个字符开头的子串，首字符 `A` 匹配，但第二个字符未知。故令 `j = 2`。

**情形4**：第3个字符失配，前2个字符为 `A B`。以第2个字符开头的子串，首字符 `B` 不匹配；以第3个字符开头的子串，首字符 `A` 匹配，但第二个字符未知。故令 `j = 1`。

**情形5**：第2个字符失配，前1个字符为 `A`。以第2个字符开头的子串，首字符 `A` 可匹配，但第二个字符未知，故令 `j = 1`（即从模式串第1个字符开始比较）。

**情形6**：第1个字符就失配，此时无法利用任何已知信息，只能让主串指针后移一位，与模式串第1个字符重新比较。在代码中，我们令 `j = 0`，然后 `i++`、`j++` 实现这一效果。

#### 三、抽象出 `next` 数组

上述所有情形下，失配时 `j` 的回退位置**仅取决于模式串本身**，与主串无关。因此，我们可以预先为每个位置 `j` 计算一个值 `next[j]`，表示当第 `j` 个字符失配时，`j` 应回退到的位置。

对于模式串 `"ABCABC"`（或其他示例），我们通过上述分析得到：
- `next[1] = 0`（特殊处理）
- `next[2] = 1`
- `next[3] = 1`（示例中可能不同，但核心是定义）
- `next[4] = 2`
- `next[5] = 2`
- `next[6] = 3`

（实际值依具体模式串而定，此处仅为说明逻辑。）
...
与朴素算法相比，改动仅两处：
1. 失配时，不再修改 `i`，只修改 `j = next[j]`；
2. 增加 `j == 0` 的特殊处理，用于第一个字符失配时后移主串指针。

#### 五、时间复杂度分析

- **预处理（求 `next` 数组）**：只需扫描模式串一次，时间复杂度为 `O(m)`，其中 `m` 为模式串长度。
- **匹配过程**：主串指针 `i` 从不回溯，最多移动 `n` 次（`n` 为主串长度），时间复杂度为 `O(n)`。
- **总体**：KMP算法的时间复杂度为 **`O(n + m)`**，远优于朴素算法最坏情况下的 `O(n*m)`。

#### 六、考研要求与实际应用

在408考试中，重点在于**手算 `next` 数组**，通常以选择题形式考查。代码实现考查较少，但需理解算法思想，能模拟执行过程。此外，需熟记时间复杂度。

朴素算法因实现简单，在失配不频繁的场景下仍被广泛使用；而KMP在模式串较长、主串指针频繁回溯时优势显著。


# 4.2.2_2_求next数组

上一小节我们学习了 KMP 算法的原理：先根据模式串求出对应的 `next` 数组，再基于该数组进行匹配，匹配过程中主串指针 `i` 不回溯。本小节将重点讲解**如何用手算方式快速求出模式串的 `next` 数组**，这是考研常考题型。

#### 一、`next` 数组的基本约定

`next` 数组的长度与模式串长度相同，下标从 1 开始。例如模式串长度为 6，则 `next[1]` 到 `next[6]` 分别对应每个位置失配时的回退位置。

**两条固定结论**（适用于任何模式串）：
- `next[1] = 0` —— 当第 1 个字符就失配时，需让 `j = 0`，然后通过特殊处理使 `i` 和 `j` 同时加 1，相当于主串指针后移一位，从模式串第 1 个字符重新开始。
- `next[2] = 1` —— 当第 2 个字符失配时，只能回退到第 1 个字符继续比较。

因此，手算时可以直接写出 `next[1] = 0`，`next[2] = 1`。

#### 二、手算 `next` 值的通用方法

对于任意位置 `j`（`j ≥ 3`），当第 `j` 个字符失配时，我们已知主串中该失配位置之前的 `j-1` 个字符与模式串的前 `j-1` 个字符完全匹配。此时，我们在失配位置前画一条**分界线**，分界线左边的字符内容是已知的，右边未知。

接下来，将模式串**向右逐步移动**，每次移动一位，并观察分界线左边的模式串前缀是否能与主串已知部分的后缀对齐。当找到**第一个能完全对齐的位置**时，该位置对应的模式串下标即为 `next[j]` 的值。若模式串一直移到完全越过分界线，则 `next[j] = 1`（即回退到模式串第 1 个字符）。

> **本质**：`next[j]` 等于由前 `j-1` 个字符组成的子串的**最长相等前后缀长度加 1**。但手算时，使用“滑动对齐法”更直观，不易出错。

#### 三、示例 1：模式串 `"gogogo"`（原例为“家狗”类，但实际字符为 `g,o,g,o,g,o` 等，此处以通用字符描述）

设模式串为 `P = "g o g o g o"`（长度 6），我们依次求 `next[1]` 至 `next[6]`。

- `next[1] = 0`，`next[2] = 1`。
- **求 `next[3]`**：若第 3 个字符（`g`）失配，前 2 个字符为 `"go"`。画分界线，将模式串右移 1 位，发现前缀 `g` 与已知的后缀 `o` 不匹配；再右移 1 位，模式串完全越过分界线，故 `next[3] = 1`。
- **求 `next[4]`**：若第 4 个字符（`o`）失配，前 3 个字符为 `"gog"`。右移 1 位，前缀 `g` 与已知后缀 `g` 匹配（分界线左边对齐），故 `next[4] = 2`（指向第 2 个字符）。
- **求 `next[5]`**：若第 5 个字符（`g`）失配，前 4 个字符为 `"gogo"`。右移 1 位，前缀 `g` 对后缀 `o`，不匹配；右移 2 位，前缀 `"go"` 对已知后缀 `"go"`，匹配，故 `next[5] = 3`。
- **求 `next[6]`**：若第 6 个字符（`o`）失配，前 5 个字符为 `"gogog"`。右移 1 位：`g` 对 `g`（匹配），但继续检查第二个字符时发现不对齐（因为已知部分更长，需完全对齐）；右移 2 位：`"go"` 对 `"og"` 不匹配；右移 3 位：`"gog"` 对 `"gog"` 匹配，故 `next[6] = 4`。

因此，该模式串的 `next` 数组为 `[0, 1, 1, 2, 3, 4]`。

#### 四、示例 2：模式串 `"ababa"`（更一般情况）

设模式串 `P = "a b a b a"`（长度 5），同样有：
- `next[1] = 0`，`next[2] = 1`。
- `next[3]`：前 2 个字符 `"ab"`，右移一位，`a` 对 `b` 不匹配，故 `next[3] = 1`。
- `next[4]`：前 3 个字符 `"aba"`，右移 1 位，前缀 `a` 与后缀 `a` 匹配，故 `next[4] = 2`。
- `next[5]`：前 4 个字符 `"abab"`，右移 1 位不匹配，右移 2 位，前缀 `"ab"` 与后缀 `"ab"` 匹配，故 `next[5] = 3`。

结果：`[0, 1, 1, 2, 3]`。

#### 五、验证：利用 `next` 数组进行匹配

以模式串 `"gogogo"` 的 `next` 数组为例，在主串中搜索该模式串时，若匹配到第 6 个字符失配，则 `j = next[6] = 4`，即模式串指针回退到第 4 个字符，主串指针 `i` 保持不变，继续比较。若回退后再次失配，则继续使用 `next[j]` 回退，直到 `j = 0` 时，执行 `i++`、`j++`，开始匹配下一个子串。

整个过程与上一小节的 KMP 匹配代码完全一致。

#### 六、补充说明与练习建议

手算 `next` 数组时，务必记住：
- `next[1] = 0`，`next[2] = 1` 是固定值。
- 从 `j = 3` 开始，每次查看失配位置之前的子串，通过“模式串右移对齐”确定最长可匹配前缀长度，再加 1（除 `next[1]` 外）。
- 若没有任何前缀能对齐，则 `next[j] = 1`。

课后建议多做几个模式串的练习（如 `"aaaaab"`、`"abcabc"` 等），并在纸上模拟匹配过程，以加深理解。KMP 算法初学不易，属于正常现象，多练多思即可掌握。


# 4.2.3_KMP算法的进一步优化

本小节将介绍KMP算法的进一步优化。经过之前的学习，我们已知KMP算法的基本原理：先根据模式串求出`next`数组，再基于该数组进行匹配。上一小节也介绍了手算`next`数组的方法。建议先完成王道书课后习题中求`next`数组的题目，再学习本节的优化内容。

接下来，我们将看到`next`数组可以如何优化，从而进一步提升KMP算法的效率。

#### 一、优化动机：避免冗余比较

以模式串`"ABABC"`（示例中为`"ABABBC"`之类，但为保持一致性，采用通用字符）为例，其`next`数组已求出。现考虑如下情况：假设主串与模式串匹配到第3个字符时失配。根据`next`数组，`j`应回退到`next[3] = 1`，即从模式串第1个字符重新开始比较。

但问题在于：刚才失配时，主串指针`i`所指的字符与模式串第3个字符`A`（假设第3个字符为`A`）不相等，因此我们可确定该主串字符**肯定不是`A`**。而`next[3]`指向的模式串第1个字符恰好也是`A`，所以接下来第1个字符的匹配**注定会再次失败**。既然注定失败，我们完全可以跳过这次比较，直接让`j`回退到`next[1] = 0`，从而减少一次无效比较。

同理，若模式串匹配到第5个字符时失配，且`next[5]`指向的字符与原失配字符相同，那么该次回退后的匹配也必然失败，应直接跳到更前的位置。

#### 二、优化规则

对于位置`j`，若`next[j]`所指向的字符**与当前失配位置`j`的字符相同**，则当`j`处失配时，回退到`next[j]`后仍会失配，因此可以直接将`next[j]`优化为`next[ next[j] ]`。若不相同，则保持原`next[j]`不变。

形式化定义优化后的数组为`nextval`：
- `nextval[1] = 0`
- 对于`j ≥ 2`：
...
步骤：
1. 先用常规方法求出原`next`数组。
2. 从`j = 2`开始，依次判断`P[j]`与`P[ next[j] ]`是否相等：
   - 若相等，则`nextval[j] = nextval[ next[j] ]`（即沿用更前位置的优化值）；
   - 若不相等，则`nextval[j] = next[j]`。
3. 特殊地，`nextval[1] = 0`。

**示例1**：模式串`"ABABA"`（长度5），其`next`数组为`[0, 1, 1, 2, 3]`。
- `nextval[1] = 0`
- `j=2`：`P[2]='B'`，`next[2]=1`，`P[1]='A'`，`B≠A`，故`nextval[2]=1`
- `j=3`：`P[3]='A'`，`next[3]=1`，`P[1]='A'`，相等，故`nextval[3] = nextval[1] = 0`
- `j=4`：`P[4]='B'`，`next[4]=2`，`P[2]='B'`，相等，故`nextval[4] = nextval[2] = 1`
- `j=5`：`P[5]='A'`，`next[5]=3`，`P[3]='A'`，相等，故`nextval[5] = nextval[3] = 0`

因此`nextval = [0, 1, 0, 1, 0]`。

**示例2**：模式串`"AAAAA"`（五个`A`），其`next`数组为`[0, 1, 2, 3, 4]`。
- `nextval[1]=0`
- `j=2`：`P[2]='A'`，`next[2]=1`，`P[1]='A'`相等 → `nextval[2] = nextval[1] = 0`
- `j=3`：`P[3]='A'`，`next[3]=2`，`P[2]='A'`相等 → `nextval[3] = nextval[2] = 0`
- 依此类推，所有`j≥2`的`nextval[j]`均为0。

故`nextval = [0, 0, 0, 0, 0]`。

**示例3**：模式串`"ABABC"`（假设字符A B A B C，长度5），其`next`数组为`[0, 1, 1, 2, 3]`（具体值依模式串而定，但逻辑相同）。
- `nextval[1]=0`
- `j=2`：`B≠A` → `nextval[2]=1`
- `j=3`：`A`与`P[1]='A'`相等 → `nextval[3] = nextval[1] = 0`
- `j=4`：`B`与`P[2]='B'`相等 → `nextval[4] = nextval[2] = 1`
- `j=5`：`C`与`P[3]='A'`不相等 → `nextval[5] = next[5] = 3`

#### 四、优化后的匹配效率

使用`nextval`数组代替原`next`数组后，KMP算法的整体逻辑不变，仅失配回退时跳过了那些注定失败的比较，从而减少了操作次数。例如，在匹配串`"AAAAAB"`等重复字符较多的场景中，优化效果显著。

#### 五、考试要求与总结

在408考研中，**手算`nextval`数组**是常见选择题考点，需熟练掌握。KMP算法整体难度较高，但实际考查分值有限，重点在于理解思想并能手动模拟。建议通过课后习题加强训练，不必过度深究代码细节。

此外，若对KMP仍有困惑，可参考王道数据结构旧版视频中的讲解，换一种思路理解。
