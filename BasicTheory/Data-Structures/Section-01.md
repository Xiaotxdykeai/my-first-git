...
![步骤](./images/PixPin_2026-07-17_22-12-20.png)
...
> 承接时间复杂度，本讲进入算法效率度量的第二部分——**空间复杂度**。核心问题：算法运行过程中占用的**内存空间**与问题规模 n 之间是什么关系？
...
| 区域 | 存放内容 | 大小是否与 n 有关 |
|------|---------|------------------|
| **代码区** | 编译后的机器指令（程序代码本身） | 固定不变，与 n 无关 |
| **数据区** | 函数参数、局部变量、递归调用信息等 | 部分与 n 有关（如数组长度依赖 n） |

> 代码区大小固定，分析空间复杂度时**只需关注数据区中与 n 相关的部分**。

## 二、空间复杂度定义

- **符号**：S(n) = O(f(n))（S = Space，空间）
- **含义**：算法运行所需内存空间与问题规模 n 之间的数量级关系
- **分析方法**：与时间复杂度类似，**只关心数量级**，使用大O表示法

## 三、空间复杂度分析的几类典型情况

### 情况1：常数阶空间复杂度 O(1) —— “原地工作”

```c
void love(int n) {
    int i = 1;       // 不管 n 多大，都只占固定4字节
    while (i <= n) {
        i++;
        printf("爱你");
    }
...
- 参数 n 占4字节，变量 i 占4字节 → 总空间 = 8字节（固定常量）
- **S(n) = O(1)**
- 若算法空间复杂度为 O(1)，称该算法可以**原地工作**（考研术语，可能考选择）

### 情况2：线性阶空间复杂度 O(n)

```c
void love(int n) {
    int flag[n];     // 长度依赖 n → 4n 字节
    int i = 1;       // 固定4字节
}
```

- 数组占 4n 字节，其他变量占常数 → S(n) = 4n + C
- **S(n) = O(n)**
...
- 三项叠加：O(n²) + O(n) + O(1) → **只保留最高阶 O(n²)**

> **规律**：多个变量占空间叠加时，只保留最高阶，与时间复杂度加法规则相同。

## 四、重点：递归调用的空间复杂度分析

> **考研常见考点**：递归算法的空间复杂度 = 递归调用的**深度** × 每层所需空间。
...
- 调用 love(5) 的递归深度 = 5 → 共5层
- 每层占用 k 字节（常数）→ 总空间 = k×n
- **S(n) = O(n)**

> 注意：即使函数内每层只定义几个变量（常数空间），递归深度为 n 时，空间复杂度依然是 O(n)，因为 n 层调用**同时存在**于内存中，不会提前释放。

### 例2：每层空间与 n 相关（更复杂）

```c
void love(int n) {
    int flag[n];      // 第1层 n 个，第2层 n-1 个，……
    if (n > 1) {
        love(n - 1);
    }
}
...
- 第1层数组长度 n，第2层 n-1，第3层 n-2，……，第n层长度1
- 总空间 = n + (n-1) + (n-2) + … + 1 = n(n+1)/2
- **S(n) = O(n²)**

>  **考试中绝大多数递归题属于例1类型**（每层常数空间），只需看递归深度即可。

### 图解递归调用时的内存变化

以 love(5) 为例（每层常数空间）：
1. 第1层调用 n=5  → 内存中存：参数5 + abc
2. 第2层调用 n=4  → 内存中存：参数4 + abc（第1层仍在）
3. 第3层调用 n=3  → 内存中存：参数3 + abc
4. 第4层调用 n=2  → 内存中存：参数2 + abc
5. 第5层调用 n=1  → 内存中存：参数1 + abc，此时条件不满足，开始返回

> 每一层的数据在返回之前都**同时存在**于内存中，所以递归空间复杂度与深度直接相关。

## 五、空间复杂度分析步骤总结

1. 找变量：识别代码中所有变量/数组/结构体
2. 分两类：
    - 与 n 无关的变量（如 int i = 1）→ 常数项，忽略
    - 与 n 有关的变量（如 int flag[n]）→ 分析其大小与 n 的关系
3. 多个变量/数组叠加时 → 取最高阶
4. 若为递归：
    - 看递归调用的深度 x 与 n 的关系
    - 若每层空间为常数，则 S(n) = O(x)；若每层空间与层数相关，则需累加
5. 套大O表示 → 得到最终空间复杂度
...
|---------|-----------|-----------|
| 衡量对象 | 执行时间 | 内存占用 |
| 符号 | T(n) = O(f(n)) | S(n) = O(f(n)) |
| 大O规则 | 相同（只保留最高阶） | 相同（只保留最高阶） |
| 考察频率 | **非常高**（每年必考） | 较低 |
| 递归分析 | 看递归执行总次数 | 看递归深度（同时存在的层数） |

## 七、重点考点提醒

| 术语/概念 | 含义 | 考查形式 |
|-----------|------|----------|
| **原地工作** | 算法空间复杂度为 O(1) | 选择题可能考 |
| **递归深度** | 递归最多同时存在多少层调用 | 空间复杂度分析的核心 |
| **大O表示法** | 只保留最高阶，忽略系数和常数项 | 同时间复杂度 |

![思维导图](./images/PixPin_2026-07-17_22-44-46.png)